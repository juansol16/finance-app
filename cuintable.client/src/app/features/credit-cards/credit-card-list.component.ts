      ...
      <div class="flex items-center justify-between mb-6">
        <div>
          ...
          <p class="text-sm text-slate-500 mt-1">
            {{ cards.length }} {{ 'COMMON.RECORDS' | translate }}
          </p>
        </div>
      ...
      >
        <div
          *ngFor="let card of cards"
          class="glass-card p-5 hover:border-white/[0.12] transition-all"
        >
          <div class="flex justify-between items-start mb-4">
            ...
            >
              {{
                card.isActive
                  ? ('CREDIT_CARD.ACTIVE' | translate)
                  : ('CREDIT_CARD.INACTIVE' | translate)
              }}
            </span>
          </div>
          ...
          <div class="flex gap-4 mb-4 text-xs" *ngIf="card.cutoffDay || card.paymentDueDay">
            <div *ngIf="card.cutoffDay" class="flex items-center gap-1.5 text-slate-400">
              <span class="w-2 h-2 rounded-full inline-block" style="background: #d97706;"></span>
              {{ 'CREDIT_CARD.CUTOFF_DAY' | translate }}:
              <span class="text-slate-300 font-medium">{{ card.cutoffDay }}</span>
            </div>
            <div *ngIf="card.paymentDueDay" class="flex items-center gap-1.5 text-slate-400">
              <span class="w-2 h-2 rounded-full inline-block" style="background: #0891b2;"></span>
              {{ 'CREDIT_CARD.PAYMENT_DAY' | translate }}:
              <span class="text-slate-300 font-medium">{{ card.paymentDueDay }}</span>
            </div>
          </div>

          <div class="flex justify-end gap-1 border-t border-white/[0.06] pt-3">
            <button
              class="btn btn-ghost btn-xs text-slate-400 hover:text-cyan-400"
              (click)="edit(card)"
            >
              {{ 'COMMON.EDIT' | translate }}
            </button>
            <button
              class="btn btn-ghost btn-xs text-slate-400 hover:text-red-400"
      ...
      <div *ngIf="showForm" class="modal modal-open">
        ...
        <div class="modal-content w-full max-w-md p-6">
          ...
          <div class="space-y-4">
            ...
            <p class="text-xs text-slate-500 -mt-2">{{ 'CREDIT_CARD.DAYS_HINT' | translate }}</p>
            <div *ngIf="editing">
          ...
          <div class="flex gap-3 mt-6">
            <button class="btn btn-ghost flex-1" (click)="showForm = false">
      ...
      <div *ngIf="deleting" class="modal modal-open">
        ...
        <div class="modal-content w-full max-w-sm p-6">
          ...
          <div class="flex gap-3">
            <button class="btn btn-ghost flex-1" (click)="deleting = null">
              {{ 'COMMON.CANCEL' | translate }}
            </button>
            <button class="btn btn-error flex-1" (click)="deleteCard()">
              {{ 'COMMON.DELETE' | translate }}
            </button>
          </div>
        </div>
